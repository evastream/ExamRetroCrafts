...
class Menu extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    /**
     * @var \Field\Megamenu\Helper\Editor
     */
    protected $editor;
     ...
     * @param \Field\Megamenu\Helper\Editor                       $editor         
    ...
    public function __construct(
        ...
        \Field\Megamenu\Helper\Editor $editor,
        \Field\Megamenu\Helper\Data $fieldData,
        $connectionName = null
        ) {
    ...
    {
        $this->_init('field_megamenu_menu', 'menu_id');
    }
    ...
    public function decodeImg($str){
        ...
        for ($i=0; $i < $count; $i++) {
            ...
            $newImg = $this->_fieldData->filter($img);
    ...
    {
        $table = $this->getTable('field_megamenu_item');
        ...
        $table = $this->getTable('field_megamenu_menu_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if($this->getStore() && $storeId=$this->getStore()->getId()){
            $select->join(
                ["cbs" => $this->getTable("field_megamenu_menu_store")],
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($menuId && ( $object->getStructure() != '' || $object->getStructure() != '[]') ) {
            $select = $this->getConnection()->select()->from($this->getTable('field_megamenu_item'))
            ->where(
                'menu_id = ?',
                (int)$menuId
                );
            ...
            if(!empty($data)){
                foreach ($data as $k => $v) {
                    ...
                    $v['htmlId'] = 'fielditem-' . $v['id'] . time() . rand();
    ...
    public function lookupStoreIds($pageId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('field_megamenu_menu_store'),
            'store_id'
            )
        ->where(
            'menu_id = ?',
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('field_megamenu_item'), $condition);
