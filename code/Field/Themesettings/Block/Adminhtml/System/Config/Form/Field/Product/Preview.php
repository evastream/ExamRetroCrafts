 ...
 */
namespace Field\Themesettings\Block\Adminhtml\System\Config\Form\Field\Product;
...
class Preview extends \Magento\Config\Block\System\Config\Form\Field
{
	...
	public function render(\Magento\Framework\Data\Form\Element\AbstractElement $element)
	{
		...
		$html .= '<div class="field-btn-preview">Close Product Preview</div>';

		$html .= '<div id="preview'.$id.'" class="field-preview">';
		$html .= '<div class="field-preview-close"><span>Close</span></div>';
		...
		$html .= 'function popShow(showLoad){
					var btnPreview = jQuery(".field-btn-preview");
					var systemConfigTabs = jQuery("#system_config_tabs");
					var leftE = (systemConfigTabs.offset().left + systemConfigTabs.width()-jQuery(".field-preview").width())+"px";
					jQuery(".field-preview").css({"left":leftE});
					if(jQuery(btnPreview).hasClass("btn-close") || showLoad){
						jQuery(btnPreview).removeClass("btn-close").html("Close Product Preview");
						jQuery(".field-preview").stop().animate({"top":"140px"});
					}else{
						jQuery(btnPreview).addClass("btn-close").html("Open Product Preview");
						jQuery(".field-preview").stop().animate({"top":"-100%"});
					}
				}
				popShow(false);
				jQuery(".field-btn-preview, .field-preview-close").on("click",function(){
					popShow();
				});';
