 ...
 */
namespace Field\Themesettings\Controller\Index;
...
class PanelTool extends \Magento\Catalog\Controller\Product\View
{
    ...
    public function __construct(
        ...
        \Field\Themesettings\Helper\Theme $themehelper,
    ...
    public function execute(){
        ...
        if(!empty($data) && $data['fieldpanel'] && !$data['fieldreset'] && $enable_paneltool){
            $this->_storeManager->setCurrentStore(1);
            $options = $data['userparams']?$data['userparams']:[];
            $this->cookieManager->deleteCookie("fieldpaneltool");
            ...
            $this->cookieManager->setPublicCookie("fieldpaneltool", serialize($options), $publicCookieMetadata);
        ...
        if($data['fieldreset']){
            ...
            $publicCookieMetadata->setHttpOnly(true);
            $publicCookieMetadata->setPath('/');
            $this->cookieManager->deleteCookie("fieldpaneltool", $publicCookieMetadata);
        }
        if(isset($data['store'])){
